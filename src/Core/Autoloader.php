<?php
/*
	Based on Laravel 3 Autoloader
*/
namespace Micro\Core;

class Autoloader
{
	/**
	 * All of the class aliases registered with the auto-loader.
	 *
	 * @var array
	 */
	public static $aliases = [];

	/**
	 * The mappings from class names to file paths.
	 *
	 * @var array
	 */
	public static $mappings = [];

	/**
	 * The mappings for namespaces to directories.
	 *
	 * @var array
	 */
	public static $namespaces = [];

	/**
	 * The directories that use the PSR-0 naming convention.
	 *
	 * @var array
	 */
	public static $directories = [];

	/**
	 * Autoloading Log array.
	 *
	 * @var array
	 */
	public static $logging = TRUE;
	public static $log = [];

	/**
	 * Инициация автозагрузки
	 */
	public static function init($config)
	{
		// Настройка
		static::aliases($config['aliases']);
		static::mappings($config['mappings']);
		static::namespaces($config['namespaces']);
		static::directories($config['directories']);

		// Bootstrap - direct class=>path mappings from previous runs

		// Use '.php' only! default extensions are: ".inc,.php".
		spl_autoload_extensions('.php');

		// Регистрируем авто-загрузчик
		spl_autoload_register([__CLASS__, 'load']);
	}

	/**
	 * Load the file corresponding to a given class.
	 *
	 * This method is registered in the bootstrap file as an SPL auto-loader.
	 *
	 * @param  string  $class
	 * @return void
	 */
	public static function load($class)
	{
		// First, we will check to see if the class has been aliased. If it has,
		// we will register the alias, which may cause the auto-loader to be
		// called again for the "real" class name to load its file.
		if (isset(static::$aliases[$class])) {
			class_alias(static::$aliases[$class], $class);
			return;
		}

		// All classes in Laravel are statically mapped. There is no crazy search
		// routine that digs through directories. It's just a simple array of
		// class to file path maps for ultra-fast file loading.
		elseif (isset(static::$mappings[$class])) {
			$path = static::$mappings[$class];
			require $path;
			static::log($class, $path);
			return;
		}

		// If the class namespace is mapped to a directory, we will load the
		// class using the PSR-0 standards from that directory accounting
		// for the root of the namespace by trimming it off.
		if(! isset($path)) {
			foreach (static::$namespaces as $namespace => $directory) {
				if (strpos($class, $namespace) === 0) {
					if ($path = static::load_psr(substr($class, strlen($namespace)), $directory)) {
						require $path;
						static::log($class, $path);
						return;
					}
				}
			}
		}

		// Try to find class in a defined directories using the PSR-0 standards
		if ($path = static::load_psr($class, static::$directories)) {
			require $path;
			static::log($class, $path);
		}
	}

	/**
	 * Attempt to resolve a class using the PSR-0 standard.
	 *
	 * @param  string  $class
	 * @param  string  $directory
	 * @param  string  $namespace	- used for logging only
	 * @return void
	 */
	protected static function load_psr($class, $directories)
	{
		// The PSR-0 standard indicates that class namespaces and underscores
		// should be used to indicate the directory tree in which the class
		// resides, so we'll convert them to slashes.
		$file = str_replace(array('\\', '_'), '/', $class);
		$lower = strtolower($file);

		// Once we have formatted the class name, we'll simply spin through
		// the registered PSR-0 directories and attempt to locate and load
		// the class file into the script.
		foreach ((array) $directories as $directory) {
			if (file_exists($path = $directory.$lower.'.php')) {
				return $path;
			}
			elseif (file_exists($path = $directory.$file.'.php')) {
				return $path;
			}
		}

		// Not found
		return;
	}

	/**
	 * Register an array of class to path mappings.
	 *
	 * @param  array  $mappings
	 * @return void
	 */
	public static function mappings($mappings)
	{
		static::$mappings = array_merge(static::$mappings, $mappings);
	}

	/**
	 * Register an array of class aliases with the auto-loader.
	 *
	 * @param  array  $aliases
	 * @return void
	 */
	public static function aliases($aliases)
	{
		static::$aliases = array_merge(static::$aliases, $aliases);
	}

	/**
	 * Register an array of directories to be searched as a PSR-0 library.
	 *
	 * @param  string|array  $directory
	 * @return void
	 */
	public static function directories($directories)
	{
		$directories = static::format_directories($directories);

		static::$directories = array_unique(array_merge(static::$directories, $directories));
	}

	/**
	 * Map namespaces to directories.
	 *
	 * @param  array   $mappings
	 * @param  string  $append
	 * @return void
	 */
	public static function namespaces($mappings, $append = '\\')
	{
		$mappings = static::format_mappings($mappings, $append);

		static::$namespaces = array_merge($mappings, static::$namespaces);
	}

	/**
	 * Format an array of namespace to directory mappings.
	 *
	 * @param  array   $mappings
	 * @param  string  $append
	 * @return array
	 */
	protected static function format_mappings($mappings, $append)
	{
		foreach ($mappings as $namespace => $directory)
		{
			// When adding new namespaces to the mappings, we will unset the previously
			// mapped value if it existed. This allows previously registered spaces to
			// be mapped to new directories on the fly.
			$namespace = trim($namespace, $append).$append;

			unset(static::$namespaces[$namespace]);

			$dirs = static::format_directories($directory);
			$namespaces[$namespace] = reset($dirs);
		}

		return $namespaces;
	}

	/**
	 * Format an array of directories with the proper trailing slashes.
	 *
	 * @param  array  $directories
	 * @return array
	 */
	protected static function format_directories($directories)
	{
		return array_map(function($directory)
		{
			return rtrim($directory, DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR;
		
		}, (array) $directories);
	}

	/**
	 * Log autoloaded classes.
	 * Helpfull for debugging and autoloaded class mapping
	 * 
	 * @param  string	$class	- autoloaded class name
	 * @param  string	$path	- class file path
	 * @return void
	 */
	protected static function log($class, $path)
	{
		if (static::$logging) {
			static::$log[$class] = $path;
		}
	}

	//-----------------------------------------------------------------------------
	//	Automapper: Autoloading class-file mapper
	//-----------------------------------------------------------------------------
	
	/**
	 * Save class map to file as php array: 'class' => 'path'.
	 *
	 * @param  string	$class	- autoloaded class name
	 * @param  string	$path	- class file path
	 * @return void
	 */
	public static function save($file, $map = NULL)
	{
		if (is_null($map)) {
			$map = static::$log;
		}

		// Generate PHP code
		$rows = [
			'<?php',
			'// Autoloading class map',
			'// Created by Micron',
			'return [',
		];
		foreach ($map as $class => $path) {
			$rows[] = "\t'".$class."' => '".$path."',";
		}
		$rows[] = '];';
		$content = implode(PHP_EOL, $rows).PHP_EOL;
//dd($rows, $content);

		// Save to PHP file
		if (file_put_contents($file, $content) === FALSE) {
			throw new \Exception("Can't save class map to file `{$file}`");
		}
	}
}